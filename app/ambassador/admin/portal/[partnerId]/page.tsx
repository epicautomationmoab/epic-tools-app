...
type Partner = { id:string; name:string; slug:string; status:string; reward_mode:string; reward_basis:string; partner_reward_cents:number; partner_reward_percent:number; guest_discount_cents:number; guest_discount_percent:number; visit_count:number; latest_visit_at:string|null; earned_cents:number; adjustment_total_cents:number; committed_cents:number; available_cents:number; recent_bookings:Booking[] };
...
function MyLink({partner}:{partner:Partner}){
  const url=referralUrl(partner.slug);
  const qrUrl=`https://quickchart.io/qr?size=320&margin=2&text=${encodeURIComponent(url)}`;
  return <div style={{display:"grid",gridTemplateColumns:"minmax(0,1.25fr) minmax(300px,.75fr)",gap:20,alignItems:"start"}}>
    <div style={{...card,padding:26}}><div style={{fontSize:12,fontWeight:900,color:"#d5521d",textTransform:"uppercase",letterSpacing:1}}>Your referral link</div><h2 style={{fontSize:26,margin:"8px 0 8px"}}>Share Epic. Earn rewards.</h2><p style={{color:"#68717d",lineHeight:1.55}}>Guests who begin their Epic visit through this link can be attributed to {partner.name}.</p><div style={{marginTop:18,display:"grid",gridTemplateColumns:"1fr auto",gap:10}}><div style={{border:"1px solid #d8dfe6",borderRadius:10,padding:"12px 14px",background:"#f8fafb",fontSize:13,overflow:"hidden",textOverflow:"ellipsis"}}>{url}</div><button onClick={()=>navigator.clipboard?.writeText(url)} style={{border:0,borderRadius:10,background:"#d5521d",color:"white",fontWeight:900,padding:"0 18px",cursor:"pointer"}}>Copy Link</button></div></div>
    <div style={{...card,padding:24,textAlign:"center"}}><div style={{fontSize:12,fontWeight:900,color:"#d5521d",textTransform:"uppercase",letterSpacing:1}}>Scan to share</div><h2 style={{margin:"8px 0 6px",fontSize:24}}>Your QR code</h2><p style={{color:"#68717d",lineHeight:1.5,margin:"0 0 16px",fontSize:13}}>Guests can scan this code to open the exact referral link.</p><div style={{display:"inline-block",padding:12,border:"1px solid #e1e5ea",borderRadius:16,background:"white",boxShadow:"0 12px 28px rgba(20,28,39,.08)"}}><img src={qrUrl} alt={`${partner.name} referral QR code`} width={260} height={260} style={{display:"block",maxWidth:"100%",height:"auto"}}/></div><div style={{fontSize:12,color:"#68717d",marginTop:13,overflowWrap:"anywhere"}}>{url}</div></div>
  </div>
}
