...
export default function LeadsTable({ leads, draftsByLead }: { leads: LeadRow[]; draftsByLead: Record<string, LeadDraft[]> }) {
    ...
    <>
      <section className={styles.tableWrap}>
        <table className={styles.table}>
          ...
          <tbody>
            {leads.map((row) => {
              const draft = (draftsByLead[row.id] || []).find(
                (item) => Number(item.tripworks_trip_id) === Number(row.primary_draft_trip_id),
              );
              return (
        ...
        {leads.length === 0 ? <div className={styles.empty}>No open future leads found.</div> : null}
      </section>

      {selected ? (
        <div className={styles.drawerBackdrop} onMouseDown={() => setSelected(null)}>
          <aside className={styles.drawer} onMouseDown={(event) => event.stopPropagation()} aria-label="Lead details">
            <div className={styles.drawerHeader}>
              ...
              <button type="button" className={styles.drawerClose} onClick={() => setSelected(null)} aria-label="Close lead details">×</button>
            </div>

            <div className={styles.drawerFacts}>
              <div><span>Lead Value</span><strong>{dollars(selected.lead_value_cents)}</strong></div>
              <div><span>Assigned Rep</span><strong>{selected.assigned_rep_name || "Unassigned"}</strong></div>
                  ...
                  return (
                    <article key={draft.id} className={`${styles.draftCard} ${isPrimary ? styles.draftCardPrimary : ""}`}>
                      <div className={styles.draftCardTop}>
                        <div>
                          <div className={styles.draftExperience}>{draft.experience_name || "TripWorks draft"}</div>
                          <div className={styles.draftOption}>{draft.option_name || "Option not supplied"}</div>
                        </div>
                        <div className={styles.draftValue}>{dollars(draft.value_cents)}</div>
                      </div>
                      <div className={styles.draftMeta}>
                        <span>{timeLabel(draft.start_time)}</span>
                        <span>{methodLabel(draft.trip_method)}</span>
                        {draft.created_by_name ? <span>Created by {draft.created_by_name}</span> : null}
                        <span>TW #{draft.tripworks_trip_id}</span>
                      </div>
