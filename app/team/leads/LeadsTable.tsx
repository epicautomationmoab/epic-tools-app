...
type CallRailMessage = { message_id: string; conversation_id: string | null; direction: string; source_number: string | null; destination_number: string | null; message_body: string | null; status: string | null; delivery_status: string | null; delivery_error: string | null; sent_at: string | null; first_received_at: string; agent_name: string | null; person_resource_id: string | null; };
...
type TimelineItem = { id: string; kind: "sms" | "call"; at: string; direction: string; body: string; meta: string; status?: string | null; error?: string | null; href?: string | null; };
type LeadActivity = { opportunity_id: string; kind: "text" | "missed_call" | "call" | "voicemail"; at: string; preview: string | null; unread: boolean; };
...
function deliveryFailed(status: string | null | undefined) { return status === "failed" || status === "error"; }
function activityLabel(activity: LeadActivity | undefined) { if (!activity?.unread) return ""; if (activity.kind === "text") return "New text"; if (activity.kind === "missed_call") return "Missed call"; if (activity.kind === "voicemail") return "Voicemail"; return "New call"; }
...
export default function LeadsTable({ leads, draftsByLead, notesByLead }: { leads: LeadRow[]; draftsByLead: Record<string, LeadDraft[]>; notesByLead: Record<string, LeadNote[]> }) {
  ...
  const [conversationMessages, setConversationMessages] = useState<CallRailMessage[]>([]);
  const [conversationCalls, setConversationCalls] = useState<CallRailCall[]>([]);
  const [conversationLoading, setConversationLoading] = useState(false);
  const [conversationError, setConversationError] = useState("");
  const [activityByLead, setActivityByLead] = useState<Record<string, LeadActivity>>({});

  const selected = useMemo(() => rows.find((row) => row.id === selectedId) || null, [rows, selectedId]);
  const repOptions = useMemo(() => [...new Set(rows.map((row) => row.claimed_by_name || row.assigned_rep_name).filter((name): name is string => Boolean(name)))].sort((a,b)=>a.localeCompare(b)), [rows]);
  const filteredRows = useMemo(() => {
    const query = searchQuery.trim().toLowerCase();
    const tokens = query.split(/\s+/).filter(Boolean);
    const queryDigits = query.replace(/\D/g, "");
    return rows.filter((row) => {
      const owner = row.claimed_by_name || row.assigned_rep_name || "";
      if (claimFilter === "unclaimed" && owner) return false;
      if (claimFilter !== "all" && claimFilter !== "unclaimed" && owner !== claimFilter) return false;
  ...
  }, [rows, draftsByLead, searchQuery, claimFilter]);

  async function loadActivity() {
    try {
      const response = await fetch("/api/team/leads/activity", { cache: "no-store" });
      const payload = await response.json();
      if (response.ok) setActivityByLead(payload.activity || {});
    } catch {}
  }
  async function markActivitySeen(opportunityId: string) {
    setActivityByLead((current) => current[opportunityId] ? { ...current, [opportunityId]: { ...current[opportunityId], unread: false } } : current);
    try { await fetch("/api/team/leads/activity", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ opportunity_id: opportunityId }) }); } catch {}
  }
  async function loadConversation(opportunityId: string, silent = false) {
    if (!silent) setConversationLoading(true);
    setConversationError("");
    try {
      const response = await fetch(`/api/team/leads?opportunity_id=${encodeURIComponent(opportunityId)}`, { cache: "no-store" });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || "Unable to load conversation history.");
      setConversationMessages(payload.messages || []);
      setConversationCalls(payload.calls || []);
    } catch (err) { if (!silent) setConversationError(err instanceof Error ? err.message : "Unable to load conversation history."); }
    finally { if (!silent) setConversationLoading(false); }
  }
  function openLead(opportunityId: string) { setSelectedId(opportunityId); setError(""); resetClose(); void markActivitySeen(opportunityId); }
  ...
  useEffect(() => {
    setSmsText(""); setSmsStatus(""); setConversationMessages([]); setConversationCalls([]); setConversationError("");
    if (!selectedId) return;
    void loadConversation(selectedId);
    const timer = window.setInterval(() => { if (document.visibilityState === "visible") void loadConversation(selectedId, true); }, 3000);
  ...
  const timeline = useMemo<TimelineItem[]>(() => {
    const items: TimelineItem[] = conversationMessages.map((message) => ({ id: `sms-${message.message_id}`, kind: "sms", at: message.sent_at || message.first_received_at, direction: message.direction || "unknown", body: message.message_body || "(No message body)", meta: message.direction === "outbound" ? `Text${message.agent_name ? ` by ${message.agent_name}` : ""}` : "Text received", status: message.delivery_status || message.status, error: message.delivery_error }));
  ...
  return <>
    <div className={styles.searchRow} style={{alignItems:"center",gap:12,flexWrap:"wrap"}}>
      <div className={styles.searchBox}><span aria-hidden="true">⌕</span><input type="search" value={searchQuery} onChange={(event)=>setSearchQuery(event.target.value)} placeholder="Search name, phone, email, TW confirmation or customer code…" aria-label="Search open leads"/>{searchQuery?<button type="button" onClick={()=>setSearchQuery("")} aria-label="Clear search">×</button>:null}</div>
      <select value={claimFilter} onChange={(event)=>setClaimFilter(event.target.value)} aria-label="Filter leads by owner" style={{minWidth:190,padding:"10px 12px",border:"1px solid #d5dce3",borderRadius:10,background:"white",font:"inherit",fontWeight:750}}><option value="all">Claimed By: All</option><option value="unclaimed">Unclaimed</option>{repOptions.map((name)=><option key={name} value={name}>{name}</option>)}</select>
      <div className={styles.searchCount}>{(searchQuery.trim()||claimFilter!=="all")?`${filteredRows.length} of ${rows.length} leads`:`${rows.length} leads`}</div>
    </div>
    <section className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Customer</th><th>Activity Window</th><th>Best Option</th><th>Method</th><th>Rep</th><th>Activity</th><th>Drafts</th><th>Lead Value</th></tr></thead><tbody>{filteredRows.map((row)=>{const draft=(draftsByLead[row.id]||[]).find((item)=>Number(item.tripworks_trip_id)===Number(row.primary_draft_trip_id));const activity=activityByLead[row.id];const label=activityLabel(activity);return <tr key={row.id} className={styles.clickableRow} onClick={()=>openLead(row.id)}><td><div className={styles.nameLine}><div className={styles.name}>{row.customer_name||"Unknown customer"}</div>{row.is_past_guest?<span className={styles.vipBadge}>VIP · Past Guest</span>:null}</div><div className={styles.contact}>{row.phone_e164||row.email||"No contact details"}</div></td><td className={styles.activity}>{dateWindowLabel(row.activity_window_start,row.activity_window_end)}</td><td><div className={styles.experience}>{draft?.experience_name||"TripWorks draft"}</div><div className={styles.contact}>{draft?.option_name||""}</div></td><td className={styles.method}>{methodLabel(row.source_method)}</td><td>{row.claimed_by_name||row.assigned_rep_name||<span className={styles.muted}>Unassigned</span>}</td><td>{label?<span title={activity?.preview||label} style={{display:"inline-block",padding:"5px 9px",borderRadius:999,fontSize:12,fontWeight:900,background:activity?.kind==="missed_call"?"#fff0ee":"#fff6d9",color:activity?.kind==="missed_call"?"#b53a31":"#6d5200",border:activity?.kind==="missed_call"?"1px solid #efb2ac":"1px solid #ebd17b"}}>{label}</span>:<span className={styles.muted}>—</span>}</td><td><span className={styles.drafts}>{row.draft_count}</span></td><td className={styles.money}>{dollars(row.lead_value_cents)}</td></tr>;})}</tbody></table>{filteredRows.length===0?<div className={styles.empty}>No open leads match those filters.</div>:null}</section>

    {selected?<div className={styles.drawerBackdrop} onMouseDown={()=>setSelectedId(null)}><aside className={styles.drawer} onMouseDown={(event)=>event.stopPropagation()} aria-label="Lead details">
      ...
      <section className={styles.drawerSection}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}><h3 style={{margin:0}}>Conversation</h3><span style={{fontSize:12,fontWeight:800,color:"#768391"}}>{selectedId?"Live · updates automatically":""}</span></div>{conversationError?<div className={styles.drawerError}>{conversationError}</div>:null}{conversationLoading&&timeline.length===0?<div className={styles.noNotes}>Loading CallRail history…</div>:timeline.length?<div style={{display:"grid",gap:10,marginTop:12}}>{timeline.map((item)=>{const outbound=item.direction==="outbound";const failed=item.kind==="sms"&&deliveryFailed(item.status);return <article key={item.id} style={{maxWidth:"82%",marginLeft:outbound?"auto":0,marginRight:outbound?0:"auto",border:failed?"1px solid #e88b83":"1px solid #dce3e9",background:failed?"#fff2f0":outbound?"#f3f7fb":"#fff",borderRadius:14,padding:"11px 13px"}}><div style={{display:"flex",gap:10,justifyContent:"space-between",fontSize:12,fontWeight:800,color:failed?"#b53a31":"#687686",marginBottom:6}}><span>{item.kind==="sms"?(failed?"Failed to send":outbound?"Epic → Customer":"Customer → Epic"):item.meta}</span><span>{dateTimeLabel(item.at)}</span></div><div style={{fontSize:15,lineHeight:1.45,whiteSpace:"pre-wrap"}}>{item.body}</div>{failed?<div style={{marginTop:7,fontSize:12,fontWeight:800,color:"#b53a31"}}>{item.error||"CallRail reported that this message failed to deliver."}</div>:item.kind==="sms"&&outbound&&item.status?<div style={{marginTop:7,fontSize:12,fontWeight:800,color:"#687686"}}>{item.status==="sent"||item.status==="delivered"?`Delivery status: ${item.status}`:"Delivery status: "+item.status}</div>:null}{item.kind==="call"&&item.href?<a href={item.href} target="_blank" rel="noreferrer" style={{display:"inline-block",marginTop:8,fontWeight:800}}>Listen to recording ↗</a>:null}</article>;})}</div>:<div className={styles.noNotes} style={{marginTop:12}}>No matched CallRail communication yet.</div>}</section>
