...
export default function LeadsTable({ leads, draftsByLead, notesByLead }: { leads: LeadRow[]; draftsByLead: Record<string, LeadDraft[]>; notesByLead: Record<string, LeadNote[]> }) {
  ...
  return <>
    <div className={styles.searchRow}><div className={styles.searchBox}><span aria-hidden="true">⌕</span><input type="search" value={searchQuery} onChange={(event)=>setSearchQuery(event.target.value)} placeholder="Search name, phone, email, TW confirmation or customer code…" aria-label="Search open leads"/>{searchQuery?<button type="button" onClick={()=>setSearchQuery("")} aria-label="Clear search">×</button>:null}</div><div className={styles.searchCount}>{searchQuery.trim()?`${filteredRows.length} of ${rows.length} leads`:`${rows.length} leads`}</div></div>
    <section className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Customer</th><th>Activity Window</th><th>Best Option</th><th>Method</th><th>Rep</th><th>Drafts</th><th>Lead Value</th></tr></thead><tbody>{filteredRows.map((row)=>{const draft=(draftsByLead[row.id]||[]).find((item)=>Number(item.tripworks_trip_id)===Number(row.primary_draft_trip_id));return <tr key={row.id} className={styles.clickableRow} onClick={()=>{setSelectedId(row.id);setError("");resetClose();}}><td><div className={styles.nameLine}><div className={styles.name}>{row.customer_name||"Unknown customer"}</div>{row.is_past_guest?<span className={styles.vipBadge}>VIP · Past Guest</span>:null}</div><div className={styles.contact}>{row.phone_e164||row.email||"No contact details"}</div></td><td className={styles.activity}>{dateWindowLabel(row.activity_window_start,row.activity_window_end)}</td><td><div className={styles.experience}>{draft?.experience_name||"TripWorks draft"}</div><div className={styles.contact}>{draft?.option_name||""}</div></td><td className={styles.method}>{methodLabel(row.source_method)}</td><td>{row.claimed_by_name||row.assigned_rep_name||<span className={styles.muted}>Unassigned</span>}</td><td><span className={styles.drafts}>{row.draft_count}</span></td><td className={styles.money}>{dollars(row.lead_value_cents)}</td></tr>;})}</tbody></table>{filteredRows.length===0?<div className={styles.empty}>{searchQuery.trim()?"No open leads match that search.":"No open future leads found."}</div>:null}</section>
