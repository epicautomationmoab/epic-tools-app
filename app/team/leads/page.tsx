...
async function getLeads() {
  ...
  const all = await supabaseRest<Array<{ id: string; status: string; captured_value_cents: number }>>(
    `sales_opportunities?select=${allSelect}`,
  );

  const openSelect = encodeURIComponent(
    "id,customer_name,email,phone_e164,activity_date,lead_value_cents,draft_count,source_method,assigned_rep_name,primary_draft_trip_id,contact_id,claimed_at,claimed_by_profile_id,claimed_by_name,is_past_guest,prior_booking_count,last_prior_booking_at",
  );
  ...
  if (openIds.length) {
    ...
    if (draftIds.length) {
      ...
      const draftSelect = encodeURIComponent(
        "id,tripworks_trip_id,confirmation_code,customer_name,email,phone_e164,activity_date,start_time,experience_name,option_name,value_cents,trip_method,created_by_name,tripworks_created_at,last_seen_at",
      );
...
export default async function LeadsPage() {
  ...
  try {
    ...
    totalOpportunities = loaded.total;
