...
export default function ServiceInboxClient({queue,title}:{queue:"rental_service"|"tour_service";title:string}){
 ...
 async function loadDetail(t:Thread){
   ...
   if(callRes){const callPayload=await callRes.json();setSms(callRes.ok?(callPayload.messages||[]):[]);setCalls(callRes.ok?(callPayload.calls||[]):[]);}else{setSms([]);setCalls([]);}
 }
 ...
 return <div className={styles.shell}>
  ...
  <section className={styles.detail}>{error?<div className={styles.error}>{error}</div>:null}{selected?<div className={styles.card}>
   ...
   {calls.length?<div className={styles.timeline}>{calls.map(call=><div key={call.id} className={`${styles.event} ${call.direction?.toLowerCase()==="outbound"?styles.outbound:styles.inbound}`}><div className={styles.eventTop}><span>{call.voicemail?"Voicemail":call.answered===false?"Missed call":call.direction?.toLowerCase()==="outbound"?"Outbound call":"Inbound call"}</span><span>{time(call.at)}</span></div>{call.duration_seconds?<strong>{Math.floor(call.duration_seconds/60)}m {call.duration_seconds%60}s</strong>:null}{call.summary||call.lead_explanation?<div className={styles.body}>{call.summary||call.lead_explanation}</div>:null}<div className={styles.actions}>{call.recording_url?<a className={styles.secondary} href={call.recording_url} target="_blank" rel="noreferrer">Play Recording</a>:null}</div>{call.transcription?<details style={{marginTop:8}}><summary style={{cursor:"pointer",fontWeight:800}}>View Transcript</summary><div className={styles.body} style={{marginTop:8}}>{call.transcription}</div></details>:null}</div>)}</div>:null}
   <div className={styles.timeline}>{timeline.map(x=><div key={x.id} className={`${styles.event} ${styles[x.type as "inbound"|"outbound"|"internal"]}`}><div className={styles.eventTop}><span>{x.label}</span><span>{time(x.at)}</span></div>{x.subject?<strong>{x.subject}</strong>:null}<div className={styles.body}>{x.body}</div></div>)}</div>
