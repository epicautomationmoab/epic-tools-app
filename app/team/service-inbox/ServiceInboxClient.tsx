...
export default function ServiceInboxClient({queue,title}:{queue:"rental_service"|"tour_service";title:string}){
 ...
 const[notes,setNotes]=useState<Note[]>([]),[transfers,setTransfers]=useState<Transfer[]>([]),[emails,setEmails]=useState<EmailEvent[]>([]),[sms,setSms]=useState<Sms[]>([]);
 ...
 async function loadDetail(t:Thread){
   const requests:[Promise<Response>,Promise<Response>,Promise<Response>?]=[
    fetch(`/api/team/service-inbox?thread_key=${encodeURIComponent(t.thread_key)}`,{cache:"no-store"}),
   ...
   const m=await meta.json(); if(meta.ok){setNotes(m.notes||[]);setTransfers(m.transfers||[]);}
   const e=await emailRes.json(); setEmails(emailRes.ok?(e.emails||[]):[]);
   if(callRes){const c=await callRes.json();setSms(callRes.ok?(c.messages||[]):[]);}else setSms([]);
 }
 ...
 useEffect(()=>{if(selected){loadDetail(selected).catch(e=>setError(e instanceof Error?e.message:"Unable to load conversation."));setReply("");setNote("");}},[selectedKey]);
 ...
 return <div className={styles.shell}>
  ...
  <section className={styles.detail}>{error?<div className={styles.error}>{error}</div>:null}{selected?<div className={styles.card}>
   <div className={styles.head}><div><div className={styles.eyebrow}>{title}</div><h2>{selected.customer_name||selected.email||selected.phone||"Unknown guest"}</h2><div className={styles.sub}>{[selected.email,selected.phone].filter(Boolean).join(" · ")}</div></div><span className={styles.pill}>{selected.reservation_confirmation||"Service"}</span></div>
