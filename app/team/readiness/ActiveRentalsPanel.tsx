...
function vehicleLabel(row: ReadinessRow) {
  const breakdown = (row.vehicle_breakdown ?? []).filter((item) => item.quantity > 0 && item.model?.trim());
  if (breakdown.length > 0) {
    return breakdown.map((item) => `${item.quantity} × ${shortVehicleModel(item.model)}`).join(", ");
  }
