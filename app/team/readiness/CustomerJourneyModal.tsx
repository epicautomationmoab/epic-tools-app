...
function readinessEvents(row: ReadinessRow, bookingMethod: string | null, bookedAt: string | null, bookingSoldBy: string | null): JourneyEvent[] {
  ...
  events.push({ id: "reservation", kind: "reservation", at: bookedAt, title: "Reservation booked", meta: [bookingMethod ? `via ${bookingMethod}` : null, bookingSoldBy ? `sold by ${bookingSoldBy}` : null, row.confirmation_code, row.product_display_name, row.business_line, row.rental_duration || null].filter(Boolean).join(" · ") });
  return events;
}
...
export default function CustomerJourneyModal({ row, onClose }: { row: ReadinessRow; onClose: () => void }) {
  ...
  const [bookingSoldBy, setBookingSoldBy] = useState<string | null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    async function load() {
      setActivityLoading(true); setActivityError(""); setBookingMethod(null); setBookedAt(null); setBookingSoldBy(null);
      try {
        ...
        if (!cancelled) {
          ...
          setBookingSoldBy(payload.booking_sold_by || null);
        }
      } catch (error) {
  ...
  const events = useMemo(() => {
    ...
    return [...readinessEvents(row, bookingMethod, bookedAt, bookingSoldBy), ...callEvents, ...textEvents].sort((a, b) => {
      if (!a.at && !b.at) return 0; if (!a.at) return 1; if (!b.at) return -1;
      return new Date(b.at).getTime() - new Date(a.at).getTime();
    });
  }, [row, calls, messages, bookingMethod, bookedAt, bookingSoldBy]);
