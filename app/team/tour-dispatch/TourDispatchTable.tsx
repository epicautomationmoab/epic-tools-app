...
function keyFor(row: TourDispatchRow) {
  return `${row.store_visit_id}:${row.vehicle_slot}`;
}
...
function departureLocked(status: string) {
...
export default function TourDispatchTable({ rows }: { rows: TourDispatchRow[] }) {
  ...
  async function saveObservation(row: TourDispatchRow) {
    ...
    if (!observation?.mpwrCar.trim() || !observation.driver) {
      setMessages((current) => ({ ...current, [key]: "Enter the MPWR car number and Driver result." }));
      return;
    }
    ...
    try {
      ...
      setStatuses((current) => ({ ...current, [key]: "out" }));
      setOpenObservation(null);
      setMessages((current) => ({ ...current, [key]: "Manual MPWR checkout recorded. Vehicle is OUT." }));
    } catch (error) {
      setMessages((current) => ({ ...current, [key]: error instanceof Error ? error.message : "Unable to save MPWR result." }));
    } finally {
      setBusyKey(null);
    }
  }
  ...
  async function releaseCheckin(row: TourDispatchRow) {
    ...
    try {
      ...
      setMessages((current) => ({ ...current, [key]: error instanceof Error ? error.message : "Unable to release check-in." }));
    } finally {
      setBusyKey(null);
    }
  }
      ...
      <table className={styles.table}>
        ...
        <tbody>
          {rows.map((row) => {
            ...
            const observation = observations[key] ?? { mpwrCar: "", driver: "expected", notes: "" };
            ...
            const observationVisible = openObservation === key;
            ...
            return (
              <>
                <tr key={key}>
                  <td><strong>{row.customer_name}</strong>{multiVehicle ? <span className={styles.slot}>Vehicle {row.vehicle_slot} of {row.total_vehicle_count}</span> : null}</td>
                ...
                {status === "checkout_queued" && observationVisible ? (
                  <tr key={`${key}:observation`} className={styles.observationRow}>
                    <td colSpan={7}>
