      ...
      <div style={{ width: "100%", maxWidth: 420 }}>
        <form onSubmit={submit} style={{ width: "100%", background: "#fff", border: "1px solid #dfe4e9", borderRadius: 16, padding: 32, boxShadow: "0 18px 50px rgba(20,31,45,.12)", boxSizing: "border-box" }}>
          ...
          <div style={{ position: "relative" }}>
            <input
              ...
              required
              style={{ width: "100%", height: 46, border: "1px solid #cfd6de", borderRadius: 9, padding: "0 46px 0 12px", boxSizing: "border-box" }}
            />
          ...
          <button
            type="button"
            onClick={() => {
              setShowPreviewAccess((value) => !value);
              setPreviewError("");
            }}
            style={{ width: "100%", marginTop: 14, border: 0, background: "transparent", color: "#475467", fontWeight: 700, cursor: "pointer" }}
          >
            {showPreviewAccess ? "Hide preview/admin access" : "Preview / Admin access"}
          </button>
        </form>
        ...
        {showPreviewAccess ? (
          <form onSubmit={submitPreview} style={{ marginTop: 14, width: "100%", background: "#fff", border: "1px solid #dfe4e9", borderRadius: 16, padding: 24, boxShadow: "0 12px 32px rgba(20,31,45,.08)", boxSizing: "border-box" }}>
            ...
            <p style={{ margin: "0 0 14px", color: "#667085", fontSize: 14 }}>Use the existing EpicTools preview password to open admin setup.</p>
            <div style={{ position: "relative" }}>
              <input
            ...
            <button type="submit" disabled={previewSubmitting} style={{ width: "100%", height: 44, marginTop: 14, border: 0, borderRadius: 9, background: "#344054", color: "#fff", fontWeight: 800, cursor: previewSubmitting ? "wait" : "pointer" }}>
              {previewSubmitting ? "Opening..." : "Open Admin Setup"}
            </button>
          </form>
        ) : null}
      </div>
