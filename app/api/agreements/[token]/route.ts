...
async function loadAgreement(token: string) {
  const rows = await supabaseSelect<AgreementRow>(
    ...
    new URLSearchParams({
      select: "id,confirmation_code,customer_name,visit_summary,amount_due_cents,tripsafe_status,policy_version,policy_title,policy_summary,policy_paragraphs,acceptance_statement,expires_at,status,accepted_at",
...
export async function GET(
  ...
  try {
    ...
    if (agreement.status !== "accepted" && new Date(agreement.expires_at).getTime() <= Date.now()) {
      await supabasePatch(
        "cancellation_agreement_requests",
