...
async function resolveSession(url: string, key: string, confirmationCode: string, publicToken: string) {
  ...
  return rows[0] as { waiver_session_id: string; waiver_template_id: string };
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (payload.p_rental_role !== "driver" && payload.p_rental_role !== "passenger") {
      return NextResponse.json({ error: "Select Driver or Passenger." }, { status: 400 });
    }
    ...
    if (payload.p_signature_method === "drawn") {
      ...
      if (!upload.ok) {
        return NextResponse.json(
          { error: `Unable to store drawn signature: ${(await upload.text()).slice(0, 300)}` },
    ...
    const rpcPayload = {
      ...
      p_rental_role: payload.p_rental_role,
      p_has_minors: Boolean(payload.p_has_minors),
      p_minors: Array.isArray(payload.p_minors) ? payload.p_minors : [],
      p_signature_method: payload.p_signature_method,
      p_typed_signature_name: payload.p_signature_method === "typed" ? payload.p_typed_signature_name : null,
      p_drawn_signature_storage_path: payload.p_signature_method === "drawn" ? storedPath : null,
    ...
    const response = await fetch(`${c.url}/rest/v1/rpc/submit_epic_rental_terms_v2`, {
      method: "POST",
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to submit rental V2 agreement." },
      { status: 500 },
    );
  }
}
