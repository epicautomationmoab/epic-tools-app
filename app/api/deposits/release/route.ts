...
export async function POST(request: Request) {
  try {
    ...
    const deposit = await callRpc<any>("get_rental_damage_deposit", {
      p_readiness_id: readinessId,
    });
    if (!deposit) return NextResponse.json({ error: "Damage deposit record not found." }, { status: 404 });

    if (deposit.status === "do_not_release") {
      if (profile.role !== "manager" && profile.role !== "admin") {
        return NextResponse.json({ error: "Manager or Admin required to release a Do Not Release deposit." }, { status: 403 });
      }

      await callRpc("set_rental_damage_deposit_hold", {
        p_readiness_id: readinessId,
    ...
    const result = await callRpc("launch_victor_release_deposit", {
      p_readiness_id: readinessId,
      p_requested_by: profile.display_name,
    });

    return NextResponse.json(result);
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to release deposit." },
      { status: 500 },
    );
  }
}
