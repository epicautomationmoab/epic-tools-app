...
async function requireManager(request: NextRequest) {
  ...
  return p && (p.role === "admin" || p.role === "manager") ? p : null;
}
function sanitize(body: Record<string, unknown>) {
  ...
  const allowed = Array.isArray(body.allowed_amounts_cents) ? body.allowed_amounts_cents.map(Number).filter((v) => Number.isFinite(v) && v > 0).map(Math.round) : [];
  return {
...
export async function GET(request: NextRequest) {
  ...
  } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to load reward catalog." }, { status: 500 }); }
}
...
export async function POST(request: NextRequest) {
  ...
  } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to create reward." }, { status: 500 }); }
}
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ ok:true, reward:rows[0] });
  } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to update reward." }, { status: 500 }); }
}
