...
export async function GET(request: NextRequest) {
  ...
  try {
    const path = partnerId
      ? `referral_partner_profiles?partner_id=eq.${encodeURIComponent(partnerId)}&select=id,partner_id,user_id,display_name,email,role,active,invited_at,last_login_at,updated_at&order=display_name.asc`
      : `referral_partner_profiles?select=id,partner_id,user_id,display_name,email,role,active,invited_at,last_login_at,updated_at&order=display_name.asc`;
    const users = await rest<Array<Record<string, unknown>>>(path);
    return NextResponse.json({ ok: true, users });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to load Ambassador users." }, { status: 500 });
  }
}
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    const rows = await rest<Array<Record<string, unknown>>>(`referral_partner_profiles?id=eq.${encodeURIComponent(profileId)}`, {
      method: "PATCH",
      headers: { Prefer: "return=representation" },
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to update Ambassador access." }, { status: 500 });
  }
}
