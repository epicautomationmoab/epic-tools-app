...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    }>>(`guest_message_templates?active=eq.true&select=${encodeURIComponent("template_id,name,message_body,sort_order,active,updated_at,updated_by")}&order=sort_order.asc,name.asc`);

    return NextResponse.json({ ok: true, can_manage: canManage(profile.role), templates });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const rows = await rest<Array<Record<string, unknown>>>("guest_message_templates", {
      ...
      body: JSON.stringify({ name, message_body: messageBody, sort_order: sortOrder, created_by: profile.display_name, updated_by: profile.display_name }),
    });
