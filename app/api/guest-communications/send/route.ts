...
async function loadNextCommunication() {
  ...
  const dueDayBeforeReminder = await fetchOneCommunication(new URLSearchParams({ select: "*", communication_type: "eq.arrival_reminder_day_before", status: "eq.scheduled", scheduled_for: `lte.${new Date().toISOString()}`, customer_email: "not.is.null", order: "scheduled_for.asc", limit: "1" }));
...
async function updateCommunication(id: string, values: Record<string, unknown>) {
  ...
  const response = await fetch(`${config.url}/rest/v1/guest_communications?id=eq.${encodeURIComponent(id)}`, {
    method: "PATCH", headers: { ...supabaseHeaders(config.key), Prefer: "return=representation" }, body: JSON.stringify({ ...values, updated_at: new Date().toISOString() }),
  });
...
export async function POST(request: Request) {
  ...
  if (communication.communication_type === "arrival_reminder_day_before" && communication.visit_date && communication.visit_date <= mountainDate()) {
    await updateCommunication(communication.id, { status: "cancelled", last_error: "Reminder became stale before sending." });
  ...
  if (communication.communication_type === "arrival_readiness_two_hour" && !hasOutstandingRequirements(portalRows)) {
    await updateCommunication(communication.id, { status: "cancelled", waiting_reason: "No outstanding guest requirements", last_error: null });
  ...
  try {
    ...
    if (communication.communication_type === "initial_guest_portal") {
      ...
      variables.CANCELLATION_POLICY_TEXT = buildCancellationPolicyText(financial.travel_protection_choice, policyDecision.status);
    }
