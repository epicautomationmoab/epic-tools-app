...
function formatVisitTime(value: string) {
  ...
  const wallTime = new Date(Date.UTC(Number(year), Number(month) - 1, Number(day), Number(hour), Number(minute)));
  const formatted = new Intl.DateTimeFormat("en-US", {
    timeZone: "UTC", weekday: "long", month: "long", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit",
  }).format(wallTime);
...
function buildReadinessMessage(rows: GuestPortalRow[]) {
  ...
  const ohvComplete = rows.every((row) => row.business_line?.trim().toLowerCase() !== "rental" || row.ohv_certificate_uploaded === true);
  if (epicReceived >= epicExpected && mpwrReceived >= mpwrExpected && ohvComplete) {
    return { headline: "You’re Ready!", message: "Nicely done—your required items are complete. Review your reservation and arrival details before your adventure." };
  }
...
async function loadCommunication(confirmationCode: string) {
  ...
  const params = new URLSearchParams({ select: "id,confirmation_code,guest_portal_token,customer_name,customer_email", communication_type: "eq.initial_guest_portal", confirmation_code: `eq.${confirmationCode}`, limit: "1" });
  const response = await fetch(`${config.url}/rest/v1/guest_communications?${params}`, { headers: supabaseHeaders(config.key), cache: "no-store" });
...
async function loadFinancialRow(confirmationCode: string) {
  const config = getSupabaseConfig();
  const params = new URLSearchParams({ select: "amount_paid_cents,amount_due_cents,total_amount_cents,travel_protection_choice,latest_payload", confirmation_code: `eq.${confirmationCode}`, limit: "1" });
  const response = await fetch(`${config.url}/rest/v1/operational_reservations?${params}`, { headers: supabaseHeaders(config.key), cache: "no-store" });
...
async function recordManualAttempt(communicationId: string, recipientEmail: string, requestedBy: string, providerMessageId: string) {
  const config = getSupabaseConfig();
  const response = await fetch(`${config.url}/rest/v1/guest_communication_attempts`, {
    method: "POST", headers: { ...supabaseHeaders(config.key), Prefer: "return=minimal" },
    body: JSON.stringify({ communication_id: communicationId, attempt_type: "manual_resend", recipient_email: recipientEmail, status: "sent", requested_by: requestedBy, provider_message_id: providerMessageId, sending_at: new Date().toISOString(), sent_at: new Date().toISOString() }),
  });
...
export async function POST(request: NextRequest) {
  if (!await canResendConfirmation(request)) return NextResponse.json({ error: "Unauthorized." }, { status: 401 });
  let body: { confirmationCode?: string };
  try { body = (await request.json()) as { confirmationCode?: string }; }
  catch { return NextResponse.json({ error: "A JSON body with confirmationCode is required." }, { status: 400 }); }
  const confirmationCode = body.confirmationCode?.trim();
  if (!confirmationCode) return NextResponse.json({ error: "confirmationCode is required." }, { status: 400 });

  try {
    const communication = await loadCommunication(confirmationCode);
    if (!communication) return NextResponse.json({ error: "No confirmation communication exists for this booking." }, { status: 404 });
    const recipientEmail = await loadEffectiveEmail(communication.confirmation_code, communication.customer_email);
    if (!recipientEmail) return NextResponse.json({ error: "The guest does not have an email address." }, { status: 409 });
    const portalRows = await loadGuestPortalRows(communication.guest_portal_token);
    if (!portalRows.length) return NextResponse.json({ error: "No guest portal rows were found." }, { status: 409 });
    const financial = await loadFinancialRow(communication.confirmation_code);
    if (!financial) return NextResponse.json({ error: "No operational reservation financials were found." }, { status: 409 });
    ...
    const profile = await getAuthenticatedTeamProfile(request.cookies.get("epic_access_token")?.value);
    await recordManualAttempt(communication.id, recipientEmail, profile?.display_name || profile?.email || "EpicTools", data.id);
    return NextResponse.json({ ok: true, confirmationCode: communication.confirmation_code, recipient: recipientEmail, providerMessageId: data.id });
  } catch (error) {
